...
This document is normative. It defines what an implementation or package must prove before claiming VAO 0.4.0 conformance.
...
5. Require the immutable 0.4.0 schema/context/version identifiers.
...
- the descriptor pins the exact manifest bytes and release ID;
...
5. successful VAO 0.4.0 SHACL validation;
...
The reversible annotation check in `vao04_rdf.py` proves only that adding/removing projection annotations preserves the parsed JSON object. It does not prove a lossless RDF-to-JSON round trip.
...
python Tools/vao04_rdf.py Fixtures/VAO04/descriptors/kinoorgel-multimodal-scientific.example.json --annotation-round-trip-check
python -m unittest discover -s tests -v
python Tools/check_release.py
```

`vao04.py validate --json` exits 0 for valid input, 1 for a completed invalid report, and 2 for invocation or operational failure. The reference validator's finite safety limits are implementation limits, not universal maximum VAO package sizes.
